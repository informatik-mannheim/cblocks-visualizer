...
const getIDFromProps = (props) => {
  return 'chart-' + props.objectID + '-' + props.instanceID + '-' + props.resourceID;
};
...
export function pinnedCharts (state = initialState, action) {
  switch (action.type) {
    ...
    case Constants.Actions.MOVE_CHART:
      return {count: state.count, all_charts: state.all_charts.map(currentChart => chart(currentChart, action))};
    case Constants.Actions.UNPIN_CHART:
      if (state.count !== 0) {
        const updatedCharts = state.all_charts.filter(item => item.chartID !== action.chartID);
